...
export default function UpdateModel(){
  ...
  useEffect(() => {
    api.getModel(id).then(res => { setForm(res.data); }).catch(()=>{}).finally(()=>setLoading(false));
  }, [id]);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow space-y-3">
        ...
        <input value={form.framework} onChange={e=>setForm({...form, framework:e.target.value})} className="w-full p-2 border rounded"/>
        <input value={form.useCase} onChange={e=>setForm({...form, useCase:e.target.value})} className="w-full p-2 border rounded"/>
        <input value={form.dataset} onChange={e=>setForm({...form, dataset:e.target.value})} className="w-full p-2 border rounded"/>
        ...
        <input value={form.image} onChange={e=>setForm({...form, image:e.target.value})} className="w-full p-2 border rounded" placeholder="Image URL"/>
